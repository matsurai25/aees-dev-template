...
class PropertyObject extends window.PropertyBaseObject {
  constructor(obj) {
    super(obj);

    // native props
    // this.propertyValueType = obj.propertyValueType;
    var value = Util.getPropertyValue(obj);
    ...
    if(obj.numKeys > 0){
      ...
      for (var i = 1; i <= obj.numKeys; i++) {
        var key = {
          time: obj.keyTime(i),
          value: Util.getPropertyValue(obj.keyValue(i))
        };
        // マーカー
        if(Util.getType(obj.keyValue(i)) !== 'Object'){
          // キーフレームだけ
...
$.global.PropertyObject = window.PropertyObject = PropertyObject;
